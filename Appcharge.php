...
if(isset($_POST["deleteop"])){
    ...
    $query=mysqli_query($conn,"DELETE FROM `charges` WHERE `id`='$id_to_delete'");
}
...
if($_SESSION["Namee"]!="Ayat"){
    $dem=$_SESSION["Namee"];
    $sql="SELECT * FROM `charges` WHERE namee ='$dem'";
}
if(isset($_POST['bing'])){
  $name_search=$_POST['searchinp'];
  $sql= "SELECT * FROM charges WHERE `namee` LIKE '%$name_search%' or `typee` LIKE '%$name_search%' or `descrip` LIKE '%$name_search%' or `netapaye` LIKE '%$name_search%' OR `prestateurvendeur` LIKE '%$name_search%'";
  $nbre_req="SELECT COUNT(id) AS nbrechg FROM `charges` WHERE `namee` LIKE '%$name_search%' or `typee` LIKE '%$name_search%' or `descrip` LIKE '%$name_search%' or `netapaye` LIKE '%$name_search%' OR `prestateurvendeur` LIKE '%$name_search%'";
}
                ...
                <div id="SuppForm" class="forms max-centring w-h90 h-b90">
                    <div class="flex justify-between w-full h-16 md:h-20">
                        <div class="w-full md:w-1/2 h-full">
                            <h4 class="font-sifonn text-2xl md:text-3xl text-slate-900">Appercus les charges</h4>
                            ...
                            <div class="md:w-full w-max">
                                ...
                                <table class="w-full text-left text-slate-900">
                                    <tr class="border font-sifonn text-base text-gray-500">
                                        ...
                                        <th class="py-4 px-3">Vendeur</th>
                                        <th class="py-4 px-3">Type de charge</th>
                                        <th class="py-4 px-3">La date</th>
                                        <th class="py-4 px-3">Description</th>
                                        <th class="py-4 px-3">Net a paye</th>
                                    </tr>
                                    <?php foreach ($charges as $charge) { ?>
                                    <tr name="<?php echo htmlspecialchars($charge['id']);?>" class="border font-sifonn text-base">
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['namee']);?></td>
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['prestateurvendeur']);?></td>
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['typee']);?></td>
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['datee']);?></td>
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['descrip']);?></td>
                                        <td class="py-4 px-3"><?php echo htmlspecialchars($charge['netapaye']);?> DH</td>
                                        <td class=""><button class="outline-none cursor-pointer w-10 h-10 p-2 bg-sky-600 rounded text-white text-sm" name="deleteop" value="<?php echo htmlspecialchars($charge['id']);?>">
                                ...
                                <h4 class=" border rounded h-14 px-3 py-4 font-sifonn text-base"><?php if(isset($_POST['bing'])){echo "le nombre des charges dans ce cas est : ".$nbre_chg;}?></div>
                                </form>
                            </div>
                        </div>
                    </div>
